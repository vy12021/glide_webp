...
public interface HeifDecoder {
   ...
   * A count of 0 ({@link HeifHeader#NETSCAPE_LOOP_COUNT_FOREVER}) means repeat indefinitely.
   ...
   *         else -1 ({@link HeifHeader#NETSCAPE_LOOP_COUNT_DOES_NOT_EXIST}).
   */
   ...
   *         <tr><td>{@link HeifHeader#NETSCAPE_LOOP_COUNT_FOREVER}</td>
   *             <td>{@link #TOTAL_ITERATION_COUNT_FOREVER}</td></tr>
   *         <tr><td>{@link HeifHeader#NETSCAPE_LOOP_COUNT_DOES_NOT_EXIST}</td>
   *             <td>{@code 1}</td></tr>
   *         <tr><td>{@code n (n > 0)}</td>
   *             <td>{@code n + 1}</td></tr>
   *     </table>
   ...
   * {@link #setData(HeifHeader, ByteBuffer)}, as well as internal buffers.
   */
  int getByteSize();
  ...
  void setData(@NonNull HeifHeader header, ByteBuffer byteBuffer);

  void setData(@NonNull HeifHeader header, ByteBuffer byteBuffer, int sampleSize);
